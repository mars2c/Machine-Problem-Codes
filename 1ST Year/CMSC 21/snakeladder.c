...
void drawPlayer(int start, int end, int direction, char (*playerArr)[4]){
   ...
   if(direction == 0){                           //Ascending order ex. 1-10
      ...
      for(ctr=0; ctr < 10; ctr++){
         for(loop=0; loop < 4; loop++){
            printf("%c", *(*(playerArr + (end - ctr)) + loop));
         }//for end
      }//for end
   }//if-else end
}//drawPlayer() end
...
void setPlayerPosition(char (*pArr)[4], Player *pstruct, int gamer, int pos){
   ...
   switch(gamer){
      case 0:  // Set the new position of player A
               ...
               *(*(pArr + oldpost) + 0) = ' ';   // Clear the old position in array        
               *(*(pArr + pos) + 0) = 'A';       // Transfer in the new position
               pstruct->A = pos;                 // Update the structure
               break;
      case 1: //Set the new position of player B
               oldpost = pstruct->B;              // Get the old position
               *(*(pArr + oldpost) + 1) = ' ';   // Clear the old position in array        
               *(*(pArr + pos) + 1) = 'B';       // Transfer in the new position
               pstruct->B = pos;                  // Update the structure
               break;
      case 2: //Set the new position of player C
               oldpost = pstruct->C;              // Get the old position
               *(*(pArr + oldpost) + 2) = ' ';   // Clear the old position in array        
               *(*(pArr + pos) + 2) = 'C';       // Transfer in the new position
               pstruct->C = pos;                  // Update the structure               
               break;
      case 3: //Set the new position of player D
               oldpost = pstruct->D;              // Get the old position
               *(*(pArr + oldpost) + 3) = ' ';   // Clear the old position in array        
               *(*(pArr + pos) + 3) = 'D';       // Transfer in the new position
...
/*   
   ...
   printf("[debug] Player A: %c\n", *(*(pArr + pos) + 0));
   printf("[debug] Struct A: %d\n", pstruct->A);
   printf("[debug] Player B: %c\n", *(*(pArr + pos) + 1));
   printf("[debug] Struct B: %d\n", pstruct->B);
   printf("[debug] Player C: %c\n", *(*(pArr + pos) + 2));
   printf("[debug] Struct C: %d\n", pstruct->C);
   printf("[debug] Player D: %c\n", *(*(pArr + pos) + 3));
   printf("[debug] Struct D: %d\n", pstruct->D);
*/
   ...
   drawStatus(playerpos, &snake, &ladder);
    ...
    while(win == 0){
        if(player == 0){                                        //Player A
            ...
            //Check new position if a snake or ladder. Update position accordingly.
            posA=0;
